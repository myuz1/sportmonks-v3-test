...
func TestNewHTTPClient(t *testing.T) {
	t.Run("instantiates with default properties", func(t *testing.T) {
		...
		assert.Equal(t, "https://api.sportmonks.com/v3/football", client.BaseURL)
	...
	t.Run("url is parsed as expected", func(t *testing.T) {
		server := newTestClient(func(req *http.Request) *http.Response {
			assert.Equal(
				t,
				req.URL.String(),
				"https://api.sportmonks.com/v3/football/continents/10?api_token=api-key&include=countries",
			)
